...
class FallingKnifeCalculator:
    ...
    def find_next_transfer_price(self):
        ...
        if should_buy_now:
            # Already in buy territory, search UP for the boundary where we stop buying
            previous_result = current_result
            for price in range(self.current_leveraged_price + 1, self.current_leveraged_price * 2):
                result = self.calculate_at_price(price)
                if result['required_transfer'] < 1000:
                    return previous_result['leveraged_price'], previous_result
