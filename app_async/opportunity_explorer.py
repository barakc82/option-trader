...
class OpportunityExplorer:
    """
    Singleton manager for identifying and executing trading opportunities.
    Coordinates between market data, account state, and the trading bot.
    """
    _instance: Optional['OpportunityExplorer'] = None

    def __new__(cls) -> 'OpportunityExplorer':
    ...
    def __init__(self) -> None:
        ...
        self.last_submit_order_attempt_time = 0.0
        ...
        self.last_put_option_price = 0.0
        self.last_call_option_price = 0.0
    ...
    def load_config(self) -> None:
        """Loads dynamic configuration from the config file."""
    ...
    async def explore_opportunities(self) -> None:
        """
        Main loop iteration for finding new trading opportunities.
        Checks for available capital, market conditions, and manages order timing.
        """
        self.load_config()
    ...
    async def _try_to_sell_side(self, open_trades: List[Trade], options: List[Contract], right: str) -> SellOptionResult:
        """Internal helper to evaluate and attempt a sell on a specific side (P/C)."""
    ...
    async def _execute_sell(self, option: Contract, quantity: float, target_delta: float) -> SellOptionResult:
        """Validates delta and executes a sell order via the trading bot."""
    ...
    async def _calculate_max_options_scenario(self, option: Contract) -> int:
        """
        Generic risk calculation for market rise (Calls) or drop (Puts).
        Projects net worth and liability under worst-case overnight move.
        """
        ...
        net_liq_after = net_liq * (fraction if right == 'P' else 1.0)
        ...
        if target_price == option.strike: return sys.maxsize
        
        liability_per_contract = abs(target_price - option.strike) * 100
        return math.floor(net_worth_after / liability_per_contract)

    def _cancel_matching_buy_trades(self, open_trades: List[Trade], option: Contract) -> None:
        """Cancels any pending buy orders for a specific contract."""
    ...
    async def _try_to_reduce_initial_margin(self, option_to_sell: Contract, sell_result: SellOptionResult, side_options: List[Contract]) -> None:
        """Attempts to buy low-cost options to relax margin requirements for a pending sell."""
    ...
    async def _calc_min_close_price(self, right: str) -> float:
        """Calculates a randomized minimal close price based on current exposure."""
        positions = await self.trading_bot.get_short_options()
...
def find_all_buy_trades(option: Contract, open_buy_trades: List[Trade]) -> List[Trade]:
    """Finds all buy trades in a list that match a specific contract."""
