...
class StateUpdater:
    ...
    async def _post_data(self, url, data):
        """Internal helper for non-blocking POST requests."""
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=data, timeout=15) as response:
                    response.raise_for_status()
        except Exception as e:
            logger.error(f"StateUpdater: Network error posting to {url}: {e}")

    def store_state_locally(self, state):
        """Atomic write to shared/state.json."""
        try:
            ...
            logger.error(f"StateUpdater: Error storing local state: {e}")
    ...
    async def update_state(self, base_state):
        ...
        try:
            ...
            logger.error(f"StateUpdater: Error updating state: {e}")
